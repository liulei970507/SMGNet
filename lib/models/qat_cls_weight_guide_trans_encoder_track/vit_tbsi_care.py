...
from lib.models.qat_cls_weight_guide_trans_encoder_track.base_backbone import BaseBackbone
from lib.models.qat_cls_weight_guide_trans_encoder_track.utils import combine_tokens, recover_tokens

# from lib.models.layers.tbsi_layer import TBSILayer
from lib.models.layers.qat_cls_weight_guide_trans_encoder_layer import QATCLSWGTELayer
from lib.models.layers.weight_pred import MS_Sigmoid
...
class VisionTransformerTBSI(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.qat_layers = nn.ModuleList()
        if self.tbsi_loc is not None and type(self.tbsi_loc) == list:
            for i in range(len(self.tbsi_loc)):
                # self.tbsi_layers.append(QATCLSLayer(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                # attn_drop=attn_drop_rate, drop_path=self.tbsi_drop_path[i], norm_layer=norm_layer, act_layer=act_layer))
                self.qat_layers.append(QATCLSWGTELayer(nn.ModuleList([MS_Sigmoid(768*2,256,1,3) for _ in range(16)]),dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=self.tbsi_drop_path[i], norm_layer=norm_layer, act_layer=act_layer))
    ...
    def forward_features(self, z, x):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            x_i_ori = x_i.clone()
            if self.tbsi_loc is not None and i in self.tbsi_loc:
                # print('self.tbsi_loc:', i)
                # x_v_ori = x_v.clone()
                # x_i_ori = x_i.clone()
                x_v, x_i, weight = self.qat_layers[qat_index](x_v, x_i)
        ...
        aux_dict = {"attn": None}
        return self.norm(x_ori), self.norm(x), aux_dict, weight_list
