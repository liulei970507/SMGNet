...
class QATMLPTrackActor(BaseActor):
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        # compute weight prediction loss
        gt_mr = torch.tensor(rounded_numbers,dtype=pred_weight[0].dtype).to(pred_weight[0].device)
        if random.random()>0.995:
            print('真值', rounded_numbers)
            print('预测', pred_weight[0].squeeze(-1).detach())
        p_tag_loss = 0.
        # import pdb
        # pdb.set_trace()
