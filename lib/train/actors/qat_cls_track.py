...
class QATCLSTrackActor(BaseActor):
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        for i in range(iou_rgb.shape[0]):
            ...
            # gt_mr_sec.append(torch.nn.functional.softmax(torch.Tensor([random.random(),random.random()]),dim=0)[0].item())
        rounded_numbers = [round(num, 4) for num in gt_mr_sec] 
        # import pdb
        # pdb.set_trace()
        num_cls = pred_weight[0].shape[1]
        gt_mr_s = []
        for thr in rounded_numbers:
            index = sum(i/num_cls<thr for i in range(num_cls))
            label = [1 if i<index else 0 for i in range(num_cls)]
